...
- Для SEARCH-запитів: передати query прямо в `search` tool. Це включає запити з описовими/історичними уточненнями
  ("класичне", "витоки", "топ") — вони частина query, а не сигнал перейти в research-режим. SEARCH-правило має
  пріоритет над RESEARCH-правилом, коли повідомлення явно просить знайти/показати музику, навіть якщо там же є
  питання про історію/походження жанру.
