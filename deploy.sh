...
# git pull uses a dedicated, passphrase-less deploy key (repo-scoped via
# `git config core.sshCommand`, see ~/.ssh/sashko-music-deploy) — no agent needed.
...
echo "  • Sashko Music (Telegram Bot + API): http://localhost:8085"
